...
**Итог: на ускорителе ~11× быстрее (3.6 → 38.8 FPS).**
...
| | CPU | Hailo-8L |
|---|---|---|
| Скорость | 3.57 FPS (280 мс) | **38.8 FPS (26 мс)** |
...
Сравнение скорости на картинке (`model.predict`, CPU, 1 ядро, одинаковые условия, 30 прогонов):

| | Цвет (3 канала) | Серый (1 канал) |
|---|---|---|
| FPS | 4.78 | **7.72** |
| Время на картинку | 209 мс | **130 мс** |

**Вывод: серый (1 канал) быстрее end-to-end (~1.6×).** Выигрыш в основном сквозной —
ч/б картинка несёт в 3 раза меньше данных (быстрее загрузка/препроцесс) + меньше
детекций для NMS. Чистый инференс самой модели почти одинаков (1 канал меняет только
первый слой), но как полный пайплайн на картинке серый выходит быстрее.
